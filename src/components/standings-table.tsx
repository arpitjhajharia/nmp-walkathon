...
export function StandingsTable({ rows, teams, full = false, highlightTeamId }: { rows: StandingRow[]; teams: Map<string, Team>; full?: boolean; highlightTeamId?: string | null }) {
  return (
      ...
      <table className="tnum w-full min-w-[520px] border-collapse text-sm">
        ...
        <tbody>
          {rows.map((r) => {
            ...
            const me = highlightTeamId === r.teamId;
            return (
              <tr key={r.teamId} className={`border-b border-line-2 last:border-0 ${me ? "bg-sky-50/60" : ""}`}>
                <td className="py-2.5 pl-4 font-display text-lg font-bold sm:pl-3">{r.position}</td>
                <td className={`sticky left-0 py-2.5 pr-2 ${me ? "bg-sky-50" : "bg-surface"}`}>
                  <TeamBadge team={team} size="sm" link />
                  {me && <span className="sr-only"> (your team)</span>}
                </td>
